...
A pair moves through six observable states:

| State | Meaning |
| --- | --- |
| `FORMING` | The pair is being bound; preconditions are being checked. |
| `COUPLED` | Active mutual coupling. |
| `AUDIT_PASSED` | The most recent integrity audit returned aligned; the pair operates with confidence. |
| `RESTORING` | Repair in progress after an aligned-but-attenuating audit. |
| `ATTENUATING` | Cadence is declining; ghosting risk if not addressed. |
