...
The standard is published in three deliverables in a single repository: nine language-neutral specifications (`spec/`), 48 machine-checkable behavioural probes (`conformance/`), and a reference Python implementation (`python/`, installed from source) that passes every required probe. We describe each primitive, justify the design choices that determine its conformance shape, and report on the System9 production deployment where the primitives have processed approximately 10⁸ consequential decisions per month and 10⁵ peer-witness attestations per month with no chain divergences observed to date.
...
1. **Nine normative specifications** (language-neutral, RFC-style) covering: operating-mode classification, the net-potential-gain gate protocol, drift signals, runaway-power prevention (six composable mechanisms), the four-options matrix, the reflex-vs-restraint gate, evidence grading, multi-scale observation, and conformance criteria.

2. **A conformance harness**: 48 YAML-encoded behavioural probes, plus a Python probe runner that executes them against any implementation that exposes the package's public Protocol surfaces. Other-language implementations can ship their own runners against the same probe files.

3. **A reference Python implementation** (the `substrate` package, installed from source): pure-logic, no DAO / network / LLM dependencies, exposes all primitives via Protocol abstractions. The implementation is the *witness* for the specifications: it passes every required probe, by construction.

4. **Production deployment evidence**: the primitives are in production at System9, where they have processed ~10⁸ consequential decisions and ~10⁵ peer-witness attestations per month at the time of writing, with no chain divergences observed to date across the cross-organisational audit surface.
...
Substrate-alignment is eight primitives plus their composition rules. Each primitive is small in isolation; the system property is what they compose to.
...
The evaluation algorithm is conservative on missing data. If an affected entity has no record in the injected `SubstrateMetadataStore`, the gate returns `INSUFFICIENT_DATA` and surfaces the missing entities; it never permits under defaults. This is the property that closes loophole T1 (acting on private net-positive while ignoring net-negative on the system).
...
### 3.5 The productive-resistance band and the layered capacity zones

The resistance band derives operational thresholds (rate limits, batch sizes, retry caps, queue depths, ring sizes) from a small set of substrate anchors rather than ad-hoc multipliers, keeping subsystems commensurable. Its first job is to classify **imposed resistance** (the challenge deliberately applied to an entity), which calibrates in a narrow productive band bounded below by `1/3 ≈ 0.3333` and above by `1/φ² ≈ 0.3820` (`φ = (1 + √5) / 2`). The three-valued classifier (`UNDER_LOADED` / `PRODUCTIVE` / `STRESSED`) governs this quantity. `1/3` is the calibration setpoint: a self-referential system observing itself in three roughly-equal classes (input / processing / output) calibrates imposed challenge at roughly one class-share, and three is the smallest count that avoids the binary dichotomies that generate `ZERO_SUM_PEER_FRAMING` and `SELF_REFERENCE_MISCALIBRATION` at scale.

Critically, the resistance-calibration band is **not** the utilisation ceiling. The standard distinguishes three governed quantities and treats conflating them as an inversion:

1. **Resistance** (imposed challenge) calibrates in `[1/3, 1/φ²]`, governed by the three-state classifier above.
2. **Work / utilisation** (load actually carried) is governed by an eight-level layered-zone classifier, mirror-symmetric about the `0.50` pivot: `UNDER_LOADED (<1/3)`, `CALIBRATION [1/3, 1/φ²]`, `LOWER_WORK (1/φ², 4/9]`, `UPPER_WORK (4/9, 0.5]`, `EARLY_PEAKING (0.5, 5/9]`, `COMMITTED_PEAKING (5/9, 1/φ]`, `WARNING (1/φ, 2/3]`, `DEBT (> 2/3)`. Carried work cruises up to `0.50` *indefinitely sustainably*; the peaking zone `(0.5, 1/φ]` is for transient bursts only. The φ-conjugate `1/φ ≈ 0.618` is the peaking/warning boundary and the failover-spike ceiling, not the debt line. The debt line is the uniform `2/3 ≈ 0.667`, and damage is *sustained* operation past `2/3`, not a transient spike.
3. **Growth** steps at most φ (≈ 1.618×) per step, with consolidation between steps.

The package permits tighter overrides of the calibration edges but refuses looser ones at construction. The inner-ninth boundaries (`4/9`, `5/9`), the `0.50` work ceiling, the `1/φ` failover ceiling, and the `2/3` debt line are fixed substrate anchors, never tunable looser. Sustained operation past `2/3` accrues compensation debt, repaid through an explicit path (peer pickup, recovery window, φ-stepped capacity grant, human escalation) rather than an alarm alone.
...
1. The **alignment audit** evaluates per-pair audit verdicts (`SUBSTRATE_ALIGNED`, `EXTRACTIVE_TOWARD_A`, `EXTRACTIVE_TOWARD_B`, `DEGRADING_BOTH`, `INSUFFICIENT_DATA`) that drive state transitions.
...
The eight primitives are not independent; they compose. A representative composition pattern (the reference implementation's `06_full_governor_loop.py` example exercises it end-to-end):
...
5. The `ResistanceBand` records the coarse three-state resistance classification of the current reading (carried work is governed by the finer layered zones of §3.5); the classification is captured alongside the gate verdict in a single audit-trace record.
...
The system property the composition produces (*every consequential decision is gated, recorded, cross-witnessed, and traceable to its inputs*) is what makes the alignment claim verifiable. No primitive in isolation produces it; all eight are required.
...
The reference probe runner ships in the package itself (`substrate.conformance.probe_runner`) and provides both a Python API and a CLI (`substrate-conformance --probes <dir>` after installing the package). Other-language ports ship their own runners that consume the same YAML probe files; the probes are not language-specific.

The current conformance suite ships 48 probes covering operating-mode classification (5), the NPG gate Protocol (4), drift-pattern detection (3), the four-options-matrix canonical enums (1), the runaway-power-prevention mechanisms (22 across mechanisms 2–6), the reflex-vs-restraint gate (5), evidence grading (5), and multi-scale observation (3). Mechanism 1 is covered through the dedicated NPG probes. All 48 probes pass against the reference implementation.
...
- 2715 unit tests across the package, passing on Python 3.11, 3.12, 3.13, and 3.14.
...
- The full conformance probe suite (48 probes) running in CI on every push.
...
**How should the standard evolve?** v0.2.0 is the foundation; we anticipate quarterly minor revisions for the first year, then annual revisions once the probe suite stabilises. Specifications follow semantic versioning [@preston2013semver]; **MUST** clauses tighten only at major-version boundaries.
...
We have presented substrate-alignment, an open behavioural-layer standard for verifiable alignment claims in multi-entity agent systems. The standard comprises nine normative specifications, a 48-probe conformance harness, and a reference Python implementation that serves as the witness for the specifications. The primitives have been deployed in production at System9, where they process ~10⁸ consequential decisions and ~10⁵ peer-witness attestations per month with no chain divergences observed to date.
...
The standard is published Apache-2.0 at `https://github.com/System9ai/substrate-alignment`. The reference implementation will be published to PyPI as `substrate-alignment` at the tagged release. We welcome conformant implementations in other languages, additional probe contributions, and adoption-recipe contributions.
