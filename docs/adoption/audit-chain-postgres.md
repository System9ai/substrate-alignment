...
class PostgresSubstrateTraceLedger:
    ...
    def _hydrate(self) -> SubstrateTraceLedger:
        ...
        in-memory ledger. ``from_records`` re-attaches rows without
        re-verifying; call the wrapper's ``verify`` (below) to run the
        package's chain walk, which detects an inconsistent chain.
        """
...
CREATE TABLE peer_witness_attestation (
    ...
    attestation     TEXT      NOT NULL,   -- "hmac-sha256:<hex>" from signer.sign()
...
```python
from substrate.audit.peer_witness_signer import HmacWitnessSigner, WitnessPayload

def witness(
    other_org_head_hash: str,
    *,
    decision_id: str,
    our_entity_id: str,
    ...
    Returns an ``"hmac-sha256:<hex>"`` attestation string. Persist it
    locally and replicate it back to the witnessed org, so both sides
    hold evidence they agree on the same head.
    """
    signer = HmacWitnessSigner(our_key)
    payload = WitnessPayload(
        decision_id=decision_id,
        primary_record_hash=other_org_head_hash,
        witness_entity_id=our_entity_id,
        witnessed_at_epoch=witnessed_at_epoch,
    )
    return signer.sign(payload)  # verify later with signer.verify(payload, attestation)
```
