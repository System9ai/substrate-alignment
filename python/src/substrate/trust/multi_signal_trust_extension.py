...
Pure-logic primitive **extending** the
:class:`SubstrateCoherenceTrustScorer` with cross-primitive multi-
signal inputs:

* **Base trust**: substrate-coherence composite score from
  the entity's substrate trace history.
* **Cadence**: coupling field-strength as relationship-
  health input.
* **Behavioral tells**: negative tell count from
  cross-entity tell detection.
* **Folk Theorem**: whether cooperation reachability
  conditions are met.
* **Peer classification**: how cross-entity peers classify
  this entity.
