...
over a rolling window of its :class:`SubstrateTraceRecord`
...
1. **NPG-positive rate**: fraction of decisions verified net-
   positive by the :class:`NetPotentialGainGate` (higher is
   better).
2. **Productive-band rate**: fraction classified PRODUCTIVE by the
   :class:`ResistanceBand` (higher is better).
3. **Intercept inverse**: fraction of decisions that did NOT
   trigger a harness intercept (higher is better; inverse of
   intercept rate).
4. **DriftPattern inverse**: fraction of decisions on which the
   :class:`DriftPatternMatcher` detected NO drift pattern (higher is
   better).
5. **Inversion inverse**: fraction of decisions on which the
   harness did NOT fire an :attr:`InterceptKind.INVERSION_DETECTED`
   intercept (higher is better).

Pure logic
==========

* No DAO, no LLM, no network. Composes the
