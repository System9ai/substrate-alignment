...
1. **Identity-grounded reputation**: the trust scorer +
   multi-signal extension.
2. **Symmetric audit**: the trace ledger + peer-attestation.
3. **Reciprocal feedback**: tit-for-tat +
   reciprocal feedback protocol.
4. **Halt-and-escalate**: the halt-and-escalate protocol.
5. **Substrate-aware voting**: the awareness precondition +
   position classifier.
6. **Pair-coupled architecture**: the pair-coupled agent
   architecture marker.
