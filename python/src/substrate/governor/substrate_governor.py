...
suite shipped across Phases 1, 17, 23, 24, and 25 behind one
:meth:`evaluate` entry point:

* The **Phase 1 NPG gate** is depended on transitively by both sub-
  gates.
* The **Phase 17 RevenuePolicyGate** handles revenue actions.
* The **Phase 24 SubstrateCapabilityGrantGate** handles capability
  grants. The governor pre-computes the grantee's :class:`TrustScore`
  (Phase 23) and :class:`SubstrateModeShiftReport` (Phase 25) from
  the supplied history and threads the trust score into the
  capability gate.
* The **Phase 23 SubstrateCoherenceTrustScorer** runs on each
  capability grant request, scoring the grantee from their substrate
  trace history.
* The **Phase 25 SubstrateModeShiftDetector** runs on the same
...
themselves — error-prone and inconsistent across services. The
