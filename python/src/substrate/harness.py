...
Per the platform's core thesis:
...
1. ``NetPotentialGainGate`` intercept on outputs that propose
   consequential actions (already-shipped primitive).
...
3. ``ReasoningModeClassifier`` Protocol, scores outputs 3D vs 5D.
   Concrete classifier ships separately.
...
6. ``ScaffoldingPolicy`` selects intercept intensity per model:
   ``LIGHT`` (host-curated local model, known pedigree), ``STANDARD``
   (bring-your-own model post-qualification), ``HEAVY`` (third-party API).
...
- **Protocols for future detectors**: ``InversionDetector`` and
  ``ReasoningModeClassifier`` are Protocols so concrete
  implementations can ship without touching this file.
...
class ScaffoldingDepth(str, Enum):
    ...
    #: A host-curated local model of known pedigree: light intercept; full
    #: substrate roles.
    LIGHT = "light"
    #: A bring-your-own model after qualification: standard intercept; full
    #: substrate roles under qualification depth.
    STANDARD = "standard"
    #: Third-party API or an unqualified bring-your-own model: heavy intercept;
    #: full substrate roles under aggressive scaffolding.
    HEAVY = "heavy"
...
# ───────────────────────────────────────────────────────────────────────
# Detector Protocols (concrete impls wired here)
# ───────────────────────────────────────────────────────────────────────

class InversionDetector(Protocol):
    """Protocol for 180° inversion detection.
...
class ReasoningModeClassifier(Protocol):
    """Protocol for 3D vs modeling mode classification.

    Returns one of the reasoning-mode labels the architecture
