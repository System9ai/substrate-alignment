...
* **SELF_REFERENCE_MISCALIBRATION**: alignment-vector miscalibration; self treated as the
  ...
  substrate. **Master pattern**, it converts every other impulse into
...
behavior trace so operators (or a broader host-specific
drift-monitoring surface, such as a structural cancer-pattern
detector) can act. **Observation, then operator.**
...
class DriftPatternReport:
    ...
    def has_findings(self) -> bool:
        """True iff at least one drift pattern was detected above threshold."""
...
class DriftPatternMatcher:  # pylint: disable=too-few-public-methods
    ...
    The matcher composes with a host-specific structural cancer-pattern
    detector as a sibling drift-vocabulary surface: that detector observes
    ...
    def _build_reasoning(
        ...
        if not detections:
            return "no drift patterns above threshold"
        parts = [
