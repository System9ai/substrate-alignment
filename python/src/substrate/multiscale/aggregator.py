"""Multi-scale substrate-state aggregator for substrate condition #3.

Pure-logic primitive making the **host application entity hierarchy** explicit
...
* **Cell** (physical, replicable, ``cell_id``): one running
  host application instance. Holds its own state. Replication and placement
  operate here.
...
class SubstrateScale(str, Enum):
    """The three scales in the host application substrate hierarchy."""
...
class CellSubstrateObservation:  # pylint: disable=too-many-instance-attributes
    ...
    Cells are **physical, replicable** instances of the host
