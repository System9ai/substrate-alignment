...
* Resistance-band distribution (under-loaded / productive /
  stressed / absent).
* Drift-pattern detection rate + per-pattern counts.
...
* Honest uncertainty. With zero records all rate properties return
  :class:`None` rather than 0.0, so operators see the absence of data
  rather than a fabricated zero.
* Composition only; consumes the typed trace record and the enums
  it references.
...
class SubstrateMetrics:  # pylint: disable=too-many-instance-attributes
    ...
    def sin_detection_rate(self) -> Optional[float]:
        """Fraction of records with any drift pattern detected."""
