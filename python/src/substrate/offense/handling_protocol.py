...
:class:`~substrate.offense.signal_type.OffenseSignalType`
, the relationship's pair-coupling state, and
...
* ``ESCALATE``: hand off to the :class:`HaltAndEscalateProtocol`
  with appropriate halt reason.
* ``DISSOLVE``: terminate the coupling via the exit protocol.
