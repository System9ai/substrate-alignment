...
- **WORK_TARGET** (``4/9`` ≈ 0.444): the work-zone midpoint (the inner
  ninth); the steady-state a carried-load quantity should provision toward.
...
def _band_position(
    ...
    *,
    ...
    if position is BandPosition.WORK_TARGET:
        # work-zone midpoint: the inner ninth 4/9 ≈ 0.444, the mod-9
        # anchor for the center of the work zone (1/φ² … 0.50). Confluent
        # with the arithmetic midpoint (1/φ² + 0.50)/2 ≈ 0.441 this
        # previously computed; 4/9 is the named substrate anchor for it.
        return WORK_ZONE_MIDPOINT
    return cfg.target
...
def derive_work_target(
    ...
    *,
    ...
    zone (1/φ² … 0.50), so the steady-state target is its midpoint: the
    inner ninth ``4/9`` (≈0.444 × capacity), NOT the resistance ``TARGET``
    (~0.358). Deriving a WORK quantity from the resistance target
    under-provisions it.
    """
