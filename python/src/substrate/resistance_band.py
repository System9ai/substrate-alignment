...
- :func:`recommend_scaling_factor`, a scalar control helper:
...
#: The **work-zone midpoint**: the inner ninth ``4/9 ≈ 0.4444``. The
#: mod-9 refinement of the thirds: ``3/9 = 1/3`` and ``6/9 = 2/3`` are the
#: outer thirds already on the ladder; ``4/9`` and ``5/9`` are the inner
#: ninths straddling the ``4.5/9 = 0.50`` pivot (a mirror pair,
#: ``4/9 + 5/9 = 1``). ``4/9`` splits the WORK zone ``(1/φ², 0.5]`` into a
#: **lower work zone** ``(1/φ², 4/9]`` (settling into cruise) and an
#: **upper work zone** ``(4/9, 0.5]`` (climbing to the pivot). It is
#: *confluent* with the arithmetic work-zone midpoint that
#: ``threshold_derivation.WORK_TARGET`` already targets (≈0.441), same
#: level, now named. A fixed substrate anchor (like the ``0.5`` pivot),
#: not config-tunable: for any valid tightened config it stays inside the
#: WORK zone (``recreation_ceiling ≤ 1/φ² < 4/9 < 0.5``).
WORK_ZONE_MIDPOINT: Final[float] = 4.0 / 9.0
...
#: The **peaking-zone midpoint**: the inner ninth ``5/9 ≈ 0.5556``, the
#: mirror of :data:`WORK_ZONE_MIDPOINT` about the ``0.50`` pivot
#: (``4/9 + 5/9 = 1``). ``5/9`` splits the PEAKING zone ``(0.5, 1/φ]``
#: into **early peaking** ``(0.5, 5/9]`` (the transient burst beginning,
#: headroom still there) and **committed peaking** ``(5/9, 1/φ]`` (deep in
#: the burst, nearing the ~62% peak, where a burst should be resolving
#: back, not settling). A fixed substrate anchor, not config-tunable: for
#: any valid config it stays inside PEAKING (``0.5 < 5/9 < 1/φ ≤ growth_ceiling``).
PEAKING_MIDPOINT: Final[float] = 5.0 / 9.0
...
class ZoneClassification(str, Enum):
    """Eight-valued layered-zone classification.
    ...
    model at inner-ninth resolution (spec ``runaway-power-prevention.md`` §4):
    ...
    - LOWER_WORK ``(1/φ², 4/9]``: the lower work level; settling into cruise.
    - UPPER_WORK ``(4/9, 0.5]``: the upper work level; climbing to the pivot.
      The inner ninth ``4/9`` splits the work band here.
    - EARLY_PEAKING ``(0.5, 5/9]``: early peaking; the transient burst
      beginning, headroom still there.
    - COMMITTED_PEAKING ``(5/9, 1/φ]``: committed peaking; deep in the burst.
      The inner ninth ``5/9`` (mirror of ``4/9``) splits the peaking band here.
    ...
    The inner ninths ``4/9`` (:data:`WORK_ZONE_MIDPOINT`) and ``5/9``
    (:data:`PEAKING_MIDPOINT`) are first-class levels; the sustained-load
    logic reads them directly. The coarser three-value
    :class:`ResistanceBandClassification` remains the projection of *this*
    (all four work/peaking levels + WARNING + DEBT → STRESSED).
    """

    # pylint: disable=duplicate-code
    # The level names are 1:1 with the runtime twin LoadZone by design
    # (wire form ↔ runtime lens); the parallel is intentional.
    UNDER_LOADED = "under_loaded"
    CALIBRATION = "calibration"
    LOWER_WORK = "lower_work"
    UPPER_WORK = "upper_work"
    EARLY_PEAKING = "early_peaking"
    COMMITTED_PEAKING = "committed_peaking"
    WARNING = "warning"
    DEBT = "debt"
...
#: The two WORK levels of :class:`ZoneClassification`, grouping for consumers
#: that reason about the coarse work band ``(1/φ², 0.50]``. Grouping, not folding.
WORK_ZONES: Final[frozenset[ZoneClassification]] = frozenset(
    {ZoneClassification.LOWER_WORK, ZoneClassification.UPPER_WORK}
)

#: The two PEAKING levels of :class:`ZoneClassification`, grouping for the
#: coarse peaking band ``(0.50, 1/φ]``.
PEAKING_ZONES: Final[frozenset[ZoneClassification]] = frozenset(
    {ZoneClassification.EARLY_PEAKING, ZoneClassification.COMMITTED_PEAKING}
)
...
_ZONE_TO_LEGACY: Final[dict["ZoneClassification", ResistanceBandClassification]] = {
    ...
    ZoneClassification.LOWER_WORK: ResistanceBandClassification.STRESSED,
    ZoneClassification.UPPER_WORK: ResistanceBandClassification.STRESSED,
    ZoneClassification.EARLY_PEAKING: ResistanceBandClassification.STRESSED,
    ZoneClassification.COMMITTED_PEAKING: ResistanceBandClassification.STRESSED,
...
def classify_zone(  # pylint: disable=too-many-return-statements
    utilization: float,
    *,
    config: Optional[ResistanceBandConfig] = None,
) -> ZoneClassification:
    """Return the eight-valued layered-zone classification.

    The calibration edges come from ``config`` (tighter-only, as with
    :func:`classify`); the inner ninths (``4/9`` work-zone midpoint, ``5/9``
    peaking midpoint), the work-zone ceiling (0.5), the φ-conjugate growth
    ceiling, and the ``2/3`` debt line are substrate anchors and are not
    tunable. Boundaries: calibration edges inclusive; LOWER_WORK is
    ``(1/φ², 4/9]``, UPPER_WORK ``(4/9, 0.5]``, EARLY_PEAKING ``(0.5, 5/9]``,
    COMMITTED_PEAKING ``(5/9, 1/φ]``, WARNING ``(1/φ, 2/3]``; DEBT is strictly
    above ``2/3``. This is the *geometric* zone; whether a DEBT-range reading
    ...
    if utilization <= WORK_ZONE_MIDPOINT:
        return ZoneClassification.LOWER_WORK
    if utilization <= WORK_ZONE_UPPER:
        return ZoneClassification.UPPER_WORK
    if utilization <= PEAKING_MIDPOINT:
        return ZoneClassification.EARLY_PEAKING
    if utilization <= PHI_CONJUGATE:
        return ZoneClassification.COMMITTED_PEAKING
