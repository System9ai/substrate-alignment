...
vote. Per Trevor's articulation in the
must be substrate-aware, independent, and given sufficient
deliberation time — otherwise voting quality collapses.

Three per-agent conditions
==========================

1. **5D reasoning-mode operation** — agent must be classified as
   :attr:`ReasoningMode.MODELING` (Phase 18).
2. **Mode-3 game-theoretic awareness confirmed** — Phase 31 verifier
...
3. **Productive resistance band** — agent must operate in
   ...
   :attr:`ResistanceBandKind.PRODUCTIVE_DEEP` per Phase 25 (substrate
   condition #9).
...
class ReasoningMode(str, Enum):
    """Vocabulary parallel to Phase 18 reasoning_mode_classifier."""
...
class ResistanceBandKind(str, Enum):
    """Vocabulary parallel to Phase 25 / resistance_band primitive."""
