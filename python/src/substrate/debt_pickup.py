...
class DebtLedger:
    ...
    def chronic_debtors(self, *, min_events: int = 3) -> List[str]:
        """Entities repeatedly carried, a structural under-capacity.
...
class PickupPlan:
    ...
    ``feasible=False`` means no peer set satisfies the constraints, so
...
class PeerPickupCoordinator:  # pylint: disable=too-few-public-methods
    ...
    constrain which peers can actually receive a given load, so callers
