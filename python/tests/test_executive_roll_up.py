...
class TestRollUp:
    ...
    def test_dominant_zone_is_modal(self) -> None:
        agg = roll_up(_cells(0.41, 0.43, 0.82, 0.2), to_scale=ExecutiveScale.RACK)
        assert agg is not None
        assert agg.dominant_zone is LoadZone.LOWER_WORK  # two cells in the lower work level
    ...
    def test_zone_distribution_in_severity_order(self) -> None:
        ...
        assert zones == [LoadZone.IDLE, LoadZone.LOWER_WORK, LoadZone.DANGER]
