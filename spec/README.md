...
| [`reflex-restraint.md`](reflex-restraint.md) | Reflex-vs-restraint gate: when a primitive halts rather than reacts |
| [`evidence-grade.md`](evidence-grade.md) | Four-step evidence-grade ladder for substrate-state claims |
| [`multi-scale.md`](multi-scale.md) | Pluggable `SubstrateScope` registry (defaults CELL/NODE/ORG) |
...
4. Run the probes that target your declared scope: `python -m substrate.conformance --probes conformance/probes/ --filter '<spec>*'`.

## Stability

The specs are at `v0.2.0-draft`. Expect editorial revision (clarifications, typos) before the first tagged release. **MUST** clauses are stable in shape (they reflect behavior that the reference implementation already exhibits and that 2700+ unit tests pin down), but their wording may tighten.
