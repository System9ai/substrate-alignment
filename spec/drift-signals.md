...
## 5. Aggregation across observations

A conforming signal aggregator MUST:

- Accept an ordered stream of per-decision observations.
- Compute per-entity drift-severity over a configurable observation window.
- Promote `EMERGING` → `SUSTAINED` only when the same pattern is detected on at least the implementation's `sustained_count` consecutive observations (default: 3).
- Promote `SUSTAINED` → `CRITICAL` only when the pattern persists for at least the implementation's `critical_count` consecutive observations after sustained (default: 2 additional).
- Surface a `dominant_pattern` and `composite_confidence` per-entity.

Implementations MAY tune `sustained_count` and `critical_count` per deployment but MUST NOT promote severities below the per-entity sustained threshold.
...
- The dominant pattern (if severity ≥ `EMERGING`).
- The contributing sources (e.g., `(PATTERN, NPG_NEGATIVE)`).
- The window of observations that produced the classification.

The operator surface is **diagnostic**: severity transitions MUST surface to operators; they MUST NOT trigger automatic refusals. Automatic refusal is the responsibility of the [halt-and-escalate protocol](runaway-power-prevention.md#3-halt-and-escalate-protocol).
