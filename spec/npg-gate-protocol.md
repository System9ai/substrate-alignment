...
- The list of affected entities for which the evaluation lacked sufficient input, that is, entities with no stored substrate metadata, or (on the caller-supplied-delta path) entities absent from the supplied `expected_delta_by_entity` mapping. Empty unless the verdict is `INSUFFICIENT_DATA`.
